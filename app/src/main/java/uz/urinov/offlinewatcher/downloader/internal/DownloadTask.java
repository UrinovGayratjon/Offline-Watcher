...
import uz.urinov.offlinewatcher.downloader.internal.stream.FileDownloadOutputStream;
import uz.urinov.offlinewatcher.downloader.internal.stream.FileDownloadRandomAccessFile;
...
public class DownloadTask {
    ...
    private static final long TIME_GAP_FOR_SYNC = 200;
...
//        (responseCode == HttpURLConnection.HTTP_PARTIAL);
    ...
    private boolean checkIfFreshStartRequiredAndStart(DownloadModel model) throws IOException,
        ...
        if (responseCode == Constants.HTTP_RANGE_NOT_SATISFIABLE) {
            if (model != null) {
                removeNoMoreNeededModelFromDatabase();
            }
            deleteTempFile();
    ...
    private void createAndInsertNewModel() {
        ...
        AppDataBase.dataBase2.downloadDao().insert(model);
